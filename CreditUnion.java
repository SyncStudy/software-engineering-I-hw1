 ...
 public class CreditUnion{
	...
	public CreditUnion(double initial)
	{
			...
			//TODO: Make sure you can't have a negative amount of initial capital
                        if(initial<0)
                        {
                            System.out.println("Credit Union does not accept negative amount of initial capital");
                        }
                        else
                        {
	 ...
	 */
	public double getTotalCapital(Person p) throws Exception
	{
		//TODO: This method should only be available to Clerks	
		if(!( p instanceof Clerk))
                {
                   throw new AccountException("Can not process total capital due to not clerk" );                
                }
                else
                {
